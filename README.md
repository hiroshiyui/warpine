...
- **Phase 4** (Filesystem I/O) — In progress. HPFS-compatible virtual filesystem with VfsBackend trait, pluggable backends (host-directory first), case-insensitive lookup, extended attributes, file locking, and sandbox isolation. Step 1 (VFS trait + DriveManager) complete.
