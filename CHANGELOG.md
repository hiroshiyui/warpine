...
### Added
- **DLL INITTERM call injection** — `_DLL_InitTerm(hmod, 0)` called at `DosLoadModule` time via vCPU call-injection; `ApiResult::CallGuest { addr, hmod, phmod }` variant added; `FrameKind` enum (`PmCallback` / `InitTerm { hmod, phmod }`) generalises `CallbackFrame` to distinguish PM window-proc callbacks from INITTERM returns; CALLBACK_RET_TRAP handler writes `*phmod` and returns `NO_ERROR` on success, `ERROR_INIT_ROUTINE_FAILED` (199) on failure; `LoadedDll::initterm_addr: Option<u32>` populated from LX `eip_object`/`eip` header fields; 2 new unit tests (`lx_loader.rs`)
