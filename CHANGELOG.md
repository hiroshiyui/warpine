...
- **Write-back path/EA strings codepage-aware** — `DosQueryCurrentDir`, `dos_enum_attribute` (DENA1), and `dos_query_path_info` (FEA2LIST) now call `cp_encode()` with the active codepage before writing strings to guest memory; `write_filefindbuf3_multi` (DosFindFirst/Next filename field) likewise encoded; ensures non-ASCII filenames and EA names are correctly re-encoded in the guest's active codepage
