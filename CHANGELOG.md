...
- **VIO output codepage-aware** — `VioManager::buffer` changed from `Vec<(u8, u8)>` to `Vec<(char, u8)>` (Unicode codepoint + attribute); new `decode_vio_byte(b, cp) -> char` helper decodes CP bytes to Unicode at write time via `cp_decode`; `get_glyph_for_char(ch) -> [u8; 16]` in `text_renderer.rs` reverse-maps Unicode chars through the CP437 table to find the correct 8×16 bitmap glyph; `VioReadCellStr` re-encodes stored chars back to active-codepage bytes via `cp_encode` before writing to guest memory; `VioWrtNCell`/`VioScrollUp`/`VioScrollDn` fill-cells also decoded through active codepage; 7 new tests (3 `decode_vio_byte`, 4 `get_glyph_for_char`); total 358 unit+integration tests pass
